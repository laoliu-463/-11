...
1.  **Volatile Memory Type / 易失性存储器类型**
    ...
    * A. ROM / 只读存储器
    * B. E2PROM / 电可擦可编程只读存储器
    * C. RAM / 随机存取存储器
    * D. flash memory / 闪存
    
    > **答案 (Answer): C**

2.  **6-Transistor Memory Structure / 6晶体管结构存储器**

    **English:** Which type of memory has 6-transistor structure?
    
    **中文：** 下列哪种类型的存储器采用6晶体管结构？
...
3.  **Flash Memory Type / 闪存类型**

    **English:** Flash memory is _______.
...
4.  **Internal Memory Properties / 内存属性**

    **English:** Which answer about internal memory is not true?
    
    **中文：** 关于内存，下列哪个说法是错误的？
    
    * A. RAM can be accessed at any time, but data would be lost when power down. / RAM可以随时访问，但断电后数据会丢失。
    * B. When accessing RAM, access time is non-relation with storage location. / 访问RAM时，访问时间与存储位置无关。
    * C. In internal memory, data can't be modified. / 在内存中，数据不能被修改。
    * D. Each addressable location has a unique address / 每个可寻址位置都有唯一的地址
    
    > **答案 (Answer): C**

5.  **Permanent Data Memory / 永久数据存储器**

    **English:** Which type of memory contains a permanent pattern of data that cannot be changed?
    
    **中文：** 哪种类型的存储器包含不能被改变的永久数据模式？
...
6.  **Nonvolatile Semiconductor Memory / 非易失性半导体存储器**
...
7.  **Memory Description / 存储器描述**

    **English:** In the following description, which is NOT right?
    
    **中文：** 下列描述中，哪个是错误的？
    
    * A. SRAM is faster than DRAM / SRAM比DRAM快
    * B. DRAM needs periodic charge refreshing circuit / DRAM需要周期性的电荷刷新电路
    * C. DRAM is slower than Flash / DRAM比闪存慢
    * D. ROM does not require power supplied continuously / ROM不需要持续供电
    
    > **答案 (Answer): C**

8.  **Nonvolatile Memory Type / 非易失性存储器类型**
