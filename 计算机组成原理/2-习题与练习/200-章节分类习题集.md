...
22. The instruction "sub R1, R2, R3"means "(R1) - (R2) → R3". After execution, this instruction generates the flags： CF and OF. If (R1) = FFFFFFFFH, (R2) = FFFFFFF0H, then after the execution of this instruction, CF and OF are ( ) respectively.
A. CF=0, OF=0
B. CF=1, OF=0
C. CF=0, OF=1
D. CF=1, OF=1
...
24.Float-type data is usually represented in the IEEE 754, a float-type variable x in a 32-bit floating-point register FR1, and x = -8.25, then the content of FR1 is ( ).
A. C1040000H B. C2420000H C. C1840000H D. C1C20000H

25.The IEEE 754 single-precision floating-point representation of -0.4375 is ( ).
A. BEE00000H B. BF600000H C. BF700000H D. C0E00000H


1Assume two integers x = -68 and y = -80 are represented in two's complement form (including 1 sign bit). x and y are stored in registers A and B. There are also two registers C and D. Registers A, B, C, and D are all 8-bit registers. Please answer the following questions (final answers should be in hexadecimal representation):
...
2.Assume the following C program is executed in an 8-bit word-length computer:
unsigned int x = 134;
unsigned int y = 246;
int m = x;
int n = y;
...
2. The address of the top of stack is stored in __________.
...
B. Program counter to contain the address of next instruction 
...
A. Operation code                 B. Source Operand reference  
C. Result Operand reference         D. Next Instruction Reference 
...
9.When the CPU executes an instruction, the address of the instruction is provided by (              ).
A. Program Counter (PC)      B. Address code field of the instruction
C. Operating System          D. Programmer
10.The function of program control instructions is ().
A. Performing arithmetic and logical operations
B. Transferring data between main memory and the CPU
C. Transferring data between the CPU and I/O devices
D. Changing the sequence of program execution
11. Among the following instructions, the one that does not belong to program control instructions is ().
A. Unconditional branch instruction     B. Conditional branch instruction
C. Implied interrupt instruction         D. Loop instruction
12.Among the following instructions, the one that applications are not allowed to use is (           ).
A. Loop instruction                B. Conversion instruction
C. Privileged instruction            D. Conditional branch instruction
13.In stack computers, some zero-address instructions do not provide the operand addresses in the instruction, the two operands come from ().
A. Accumulator and register
B. Accumulator and temporary register
C. Top and second top units of the stack
D. Top unit of the stack and a temporary register
14.Assuming a machine word length of 32 bits and a memory capacity of 16MB, if the CPU addresses by half-word, the number of addressable units is ().
A. 224        B. 223          C. 222           D. 221
15.An instruction set has 200 instructions. Using fixed-length binary encoding for the opcode, a minimum of () bits is required.
A. 4            B. 8             C. 16         D. 32
16.In the instruction format, the purpose of adopting the expanding opcode design scheme is (           ).
A. To reduce the instruction word length
B. To increase the instruction word length
C. To increase the number of instructions while keeping the instruction word length unchanged
D. To increase the addressing space while keeping the instruction word length unchanged
17.A computer system uses 32-bit single-word-length instructions with a 12-bit address code. If 250 two-address instructions are defined, then the number of possible one-address instructions is ().
A. 2¹²          B. 2¹³            C. 2¹               D. 3 × 2¹³

18.A computer uses byte addressing, has a fixed instruction word length and only two instruction formats. Among them, there are 29 three-address instructions and 107 two-address instructions. Each address field is 6 bits. The instruction word length should be at least (      ). 
A. 24 bits     B. 26 bits         C. 28 bits    D. 32 bits
19.When designing an instruction system, assume a 16-bit fixed-length instruction word format is used, with the opcode using an expanding encoding scheme. The address code is 6 bits long, and there are three instruction formats: zero-address, one-address, and two-address. If there are 12 two-address instructions and 254 one-address instructions, then the maximum number of zero-address instructions is (       ).
A. 0             B. 2             C. 64              D. 128

Chapter 13 Addressing Mode
...
4. The disadvantage of INDIRECT ADDRESSING is ____________.
A. large addressing range             B. no memory access  
C. more memory access              D. large value range 
5. The disadvantage of IMMEDIATE ADDRESSING is ___________.
A. limited address range          B. more memory access  
C. limit value range              D. less memory access 
6. In register indirect addressing, the effective address of operand is stored in __   __.
A. register     B. main memory    C. instruction     D. PC 
7. The REGISTER ADDRESSING has very less address space, but it has (is) _________________.
A. very less value range          B. very fast 
C. more memory access          D. complex address calculation 

8.To reduce the number of bits in a specific address field within an instruction, an effective method is to use ( ).
A. Immediate addressing        B. Indexed addressing
C. Indirect addressing          D. Register addressing
9. A computer uses a 16-bit fixed-length instruction word format. The number of bits for the opcode and the addressing mode is fixed. The instruction set has 48 instructions and supports four addressing modes: direct, indirect, immediate, and relative. In a single-address instruction, the addressable range using direct addressing is ( ).
A. 0–255        B. 0–1023      C. -128–127          D. -512–511
10. Among the following addressing modes, which one is most suitable for accessing elements of an array in sequential order by index（下标）?
A. Relative addressing             B. Register addressing
C. Direct addressing               D. Indexed addressing
11.The format of a instruction is shown below. M represents the addressing mode, I is the index register, and D is the formal address. If the addressing mode of "indexing first, then indirection" is used, the effective address of the operand is ( ).
A. I + D          B. (I) + D          C. ((I) + D)    D. ((I)) + D
12.The displacement addressing generates an effective address by adding the content of a register to a formal address. Which of the following addressing modes does not belong to offset addressing? (  )
A. Indirect addressing       B. Base addressing
C. Relative addressing       D. Indexed addressing
13. A computer has a word length of 16 bits, and the main memory is word-addressable. A branch instruction consists of two bytes and uses relative addressing. The first byte is the opcode field, and the second byte is the displacement field. If a specific branch instruction is located at main memory address 4000H and the content of its displacement field is 06H, then the value of the PC after executing this branch instruction is ( ).
A.4002H       B.4004H            C.4007H              D.4008H
14.Assume the first operand of a certain instruction uses register indirect addressing. The register number is 8. The content of register 8 is 1200H. The content of the memory unit at address 1200H is 12FCH. The content of the memory unit at address 12FCH is 38D8H. The content of the memory unit at address 38D8H is 88F9H. The effective address of this operand is ( ).
A. 1200H         B. 12FCH         C. 38D8H           D. 88F9H
15. Assume the value in register R is 200. The contents of memory address 200 and 300 are 300 and 400, respectively. Under which addressing mode will the operand accessed be 200? ( )
A. Direct addressing 200                     B. Register indirect addressing (R)
C. Memory indirect addressing (200)           D. Register addressing R
16.Among the instruction addressing modes, the fastest way to obtain an operand is ( ).
A. Direct addressing             B. Immediate addressing
C. Register addressing            D. Indirect addressing
...
1~2章
1. Gordon Moore observed that the number of transistors that could be put on a single chip was doubling every ________ months in the 1970s.         
A. 24     B. 12    C. 18   D. 15
...
第3章
1. Which of the following statements about the basic concepts of the von Neumann architecture is incorrect? (   )
A. Program functions are realized by the CPU executing instructions
B. Both instructions and data are represented in binary, with no formal distinction
C. Instructions are accessed by address, and data are directly provided within the instructions
D. Before program execution, instructions and data must be stored in memory
2. PC holds _______________ .
A. address of next instruction    B. next instruction  
C. address of operand          D. operand
5. A unsigned binary number is n bits, so it can represent a value in the range between _________ .
...
D. More than one module may control bus at one time
15.If a computer has a word length of 4B, it indicates that the machine (   ).
A. Can process 4-bit decimal digits
B. Can process 4-bit binary digits
C. Can process 32-bit binary code
D. The maximum result of operations in the CPU is 2³²
16.A complete computer system should include (    ).
A. ALU, memory, CU
B. Peripheral devices and host
C. Host and application programs
D. Supporting hardware devices and software system 
第4章
第一部分：教材习题
1.The computer memory system refers to _________
A. RAM                  B. ROM
C. Main memory           D. Register , main memory, cache, external memory
...
A)associative mapping                 B)  direct mapping
C)set associative mapping              D)  random mapping
8. On address mapping of cache, the data in any block of main memory can be mapped to any line of the fixed set of cache. It is _________________.
A）associative mapping               B)  direct mapping
C)  set associative mapping              D)  random mapping
...
B. The addressing range 216 
C. The max main memory capacity is 216
...
A. write-back   B. write-through   C. write-invalidate   D. write-update

第二部分
1. Assuming machine word length is 32 bits, for a storage capacity of 16 MB, if CPU uses half-word addressing, the number of addressable units is ().
A． 224               B. 223          C.222         D.221
设机器字长为32位，一个容量为16MB的存储器，CPU 按半字寻址，其可寻址的单元
数是()。
A． 224               B. 223          C.222         D.221
2.In the following memory, CPU cannot directly access is ().
A. Hard disk     B. Main memory      C. Cache      D. Register
3.Consider a machine with word addressable memory of 128 MB and word length is 64 bits,  the number of addressable units is (  ).
A. 16MB      B. 16M      C. 32M    D. 32MB
设机器字长为64位，存储容量为128MB,  若按字编址，它可寻址的单元个数是()。
A.16MB         B.16M                  C.32M                      D.32MB

4.The reason computers use a hierarchical memory system is ().
A. To facilitate programming
B. To resolve the conflicts between capacity, speed, and cost
C. To facilitate the storage of large amounts of data
D. To facilitate operation
计算机的存储器采用分级方式是为了()。
A. 方便编程                         B.   解决容量、速度、价格三者之间的矛盾
C.   保存大量数据方便               D.  操作方便

5. In the memory system, the "Cache-Main Memory" structure solves the problem of ().
A. Insufficient main memory capacity
B. Speed mismatch between main memory and secondary storage
C. Speed mismatch between secondary storage and the CPU
D. Speed mismatch between main memory and the CPU
在多级存储体系中，“Cache- 主存”结构的作用是解决()的问题。
A.   主存容量不足                      B.   主存与辅存速度不匹配
C.   辅存与CPU  速度不匹配            D.   主存与CPU  速度不匹配

6. In the memory system, the correct order of storage devices from fastest to slowest is ().
A. Registers - Main Memory - Cache - Secondary Storage
B. Registers - Main Memory - Secondary Storage - Cache
C. Registers - Cache - Secondary Storage - Main Memory
D. Registers - Cache - Main Memory - Secondary Storage
存储器分层体系结构中，存储器从速度最快到最慢的排列顺序是()。
A.  寄存器-主存-Cache-辅存         B.    寄存器-主存一辅存-Cache
C.   寄存器 -Cache 一辅存-主存           D.   寄存器 -Cache-  主存一辅存

7.Which of the following statements about the multi-level storage system are correct?
I. The multi-level storage system is designed to reduce costs.
II. In virtual memory, the data transfer between main memory and auxiliary memory is transparent to all programmers.
III. The CPU can only exchange information directly with the cache, and the exchange of information between the CPU and main memory also needs to go through the cache.
A. Only I
B. Only I and II
C. I, II, and III
D. Only II
...
A. 仅I                    B.    仅 I 和 Ⅱ       C.I 、Ⅱ     和  Ⅲ      D.   仅  Ⅱ
解释：答案A主存和辅存之间的数据调动是由硬件和操作系统共同完成的，仅对应用级程序员透明。 CPU与主存可直接交换信息。
8. Suppose a Cache has M lines, and a set has K lines. Which of the following descriptions is correct? 
...
假设一个Cache 中共有M 块，每K 块组成一个组，则下列描述中正确的是()。
A.  若 K=1,     则该Cache 是直接映射Cache
B.  若 K=1,     则该 Cache 是全相联映射Cache
C.   若 K=M,    则该 Cache 是直接映射 Cache
D.  若 K>1   且K<M,     则该 Cache 是M/K 路组相联映射 Cache

9. An 8-way set-associative mapping Cache is 128KB, with 16B per line. The byte with address 1234567H is loaded into this Cache, its Tag should be ( ).
A. 1234H     B. 2468H     C. 048DH     D. 12345H
有效容量为128KB 的 Cache,   每块16B,  采用8路组相联。字节地址为1234567H的 单 元调入该Cache,   则其Tag 应 为 ( ) 。
A.1234H             B.2468H              C.048DH             D.12345H

10. For an n-way set-associative Cache, if both the main memory block size and the Cache line size are doubled, which of the following descriptions is correct? ( )
A.Word number increases 1 bit , and the tag increases 1 bit.
B. Word number increases 1bit, and the tag remains unchanged.
C. Word number decreases 1 bit, and the tag increases 1 bit.
D. Word number increases 1bit, and the tag decreases 1 bit.
对于n 路组相联映射Cache,   在保持n 及主存和Cache 总容量不变的前提下，将主存块 大小和Cache 块大小都增加一倍，则下列描述中正确的是()。
A.  字块内地址的位数增加1位，主存tag 字段的位数增加1位
B.  字块内地址的位数增加1位，主存tag 字段的位数不变
C. 字块内地址的位数减少1位，主存tag字段的位数增加1位
D.  字块内地址的位数增加1倍，主存tag 字段的位数减少一半

11. A computer's Cache has 16 lines, with a block size of 16B. Its mapping method can be either direct-mapped or 2-way set-associative. Main memory is byte-addressable, and main memory units are starting from 0. If the following main memory addresses are accessed sequentially, which pair may cause a Cache conflict regardless of which mapping method is adopted? ( )
A. address 52 and 102      B. address 48 and 308  
C. address 60 and 160      D. address 46 and 236
某计算机的Cache 有16行，块大小为16B,  其映射方式可配置为直接映射或2路组相 联映射，主存按字节编址，主存单元从0开始编号。若依次访问下列主存单元，则不论 采取上述哪种映射方式都可能引起Cache 冲突的是()。
A.52   号和102号单元               B.48    号和308号单元
C.60    号和160号单元               D.46   号和236号单元

12.Assuming main memory is byte-addressable, the Cache has a total of 64 lines, and uses a 4-way set-associative mapping method. The main memory block size is 32 bytes, and all numbering starts from 0. Into what set would the storage unit with the address 2593 be stored?
A. 1             B. 15             C. 14                D. 4
假设主存按字节编址，Cache 共有64行，采用4路组相联映射方式，主存块大小为32字节，所有编号都从0开始。则第2593号存储单元所在主存块的Cache 组 号 是 ( ) 。
A.1                  B.  15                    C. 14                   D.4

13. Regarding Cache replacement algorithms, which of the following statements is incorrect? ( )
A. Both set-associative and associative mapping must consider how to perform replacement.
B. The FIFO algorithm does not require recording replacement information for each Cache line.
C. Direct mapping does not require considering the replacement problem.
D. The LRU algorithm requires recording replacement information for each Cache line.

13.  若计算机按字编址，Cache 数据区容量为8K字，主存块大小为512字，主存地址空间为 1M 字，采用2路组相联映射方式。每次根据主存地址访问Cache 时，需要同时进行 ( ) 次tag位的比较，每次需要比较的位数是()。
A.2,8               B.2,16               C.4,8                D.4,16
13.If a computer is word-addressable, the Cache has 8K words, the main memory block size is 512 words, the main memory address space is 1M words, and a 2-way set-associative mapping method is used. When the Cache is accessed based on a main memory address, it is necessary to simultaneously perform ( ) comparisons of tag bits, and the number of bits that need to be compared each time is ( ).
A. 2, 8             B. 2, 16      C. 4, 8            D. 4, 16
14. 【2009统考真题】某计算机的Cache共有16块，采用2路组相联映射方式(每组2块)。 每个主存块大小为32B, 按字节编址，主存129号单元所在主存块应装入的Cache 组号是 ( ) 。
A.0                B. 2                   C.4                D.6
14. A  2-way set-associative Cache has 16 lines. Main memory block is 32B, and memory is byte-addressable. Which set is the word from memory location 129 mapped?
A. 0                B. 2            C. 4               D. 6
15.【2022统考真题】若计算机主存地址为32位，按字节编址，某 Cache 的数据区容量为
32KB, 主存块大小为64B, 采用8路组相联映射方式，该 Cache中比较器的个数和位数 分别为()。
A.8,20                B.8,23                    C.64,20                  D.64,23
15. If a byte-addressable main memory address is 32 bits,  the 8-way set-associative Cache has 32KB, the main memory block size is 64B. The number and bit width of comparators in this Cache are respectively ( ).
A. 8, 20        B. 8, 23            C. 64, 20              D. 64, 23
大题
一、 A set-associative cache consists of 2k lines, or slots, divided into four-line sets. Main memory contains16M blocks of 16words each.
1.Show the format of main memory addresses.
2.Which set of cache is the data from memory location DEF5C6AH mapped to? what is the tag? (when read from memory).
3.Why is the tag also stored in the cache?
答案：
1. (8points)
(1) Line size=Block size=16words=24words =>Word# =4bits
   (2) Number of sets=2klines/ 4lines/set =29sets =>Set# =9bits
(3) Memory size=16M x 16words=228words => Length of RA =28 bits
(4)Tag=28-4-9=15 bits
 
Tag	Set#	              Word#	
15	9	4       bits	
...
 
2.DEF5C6AH
  110,1111,0111,1010 1,1100,0110 1010B
set#=1C6H, tag=6F7AH(5points)
 
3. tag is used to identify which   block of main memory is currently being stored   in each cache line(2points)
 

二、A  four-way set-associative cache has  4K words and a line size of four  words. The main memory has 4G words. Questions:
1.Show the mapping format of main memory addresses. 
2.Where in the cache is the word from memory location ABCDE8F8H mapped?
What are the addresses of the other words stored along with the location ABCDE8F8H? 
3.How many blocks of main memory may be mapped to a certain line of cache memory?

答案：
1.
(1) Line size=Block size=4 words =22 words =>Word# =2bits
      (2) Number of sets=4kwords/ 4 words /4lines/set =28sets =>Set# =8bits
(3) main memory size=4Gwords=232 words =>Length of RA =32 bits 
(4) Tag=32-8-2=22 bits
Tag	set#	Word#	
22	8	2	bits
 
   2.  ABCDE8F8 H
 Binary: 10,1010,1111,0 011,0111,1010|0011,1110|00  
   word # =0H   Set# : 3EH        Tag: 2AF37AH  
 main memory Location ABCDE8F8H is mapped: tag=2AF37AH; set=3EH
the same block,word#=00~11=>other addresses: ABCDE8F9H,ABCDE8FAH,ABCDE8FBH
3） 2tag = 222 =4Mblocks
...
3. Flash memory is       .
A. read-only memory                B. read-mostly memory   
C. sequential-access memory          D. volatile
4. Which answer about internal memory is not true?
...
5. Which type of memory contains a permanent pattern of data that cannot be changed?        .
A SRAM       B DRAM        C EEPROM     D PROM
6. Which is the nonvolatile semiconductor memory? _____________.
A. SRAM     B. CD-ROM    C. FLASH    D. DRAM
7. In the following description, which is NOT right? _____________.
...
8. Which type of memory is nonvolatile?
A. Register    B. DRAM   C. cache     D. flash memory
某存储器容量为32K×16 位 ， 则 (C ) 。
A.  地址线为16根，数据线为32根    B.  地址线为32根，数据线为16根
C.   地址线为15根，数据线为16根    D.   地址线为15根，数据线为32根
9. A memory has a capacity of 32K×16 bits. Which of the following descriptions is correct?
A. It has 16 address lines and 32 data lines    B. It has 32 address lines and 16 data lines
C. It has 15 address lines and 16 data lines    D. It has 15 address lines and 32 data lines

下面是有关DRAM 和 SRAM 存储器芯片的叙述：
I.DRAM    芯片的集成度比SRAM 芯片的高
II.DRAM    芯片的成本比 SRAM 芯片的高
Ⅲ.DRAM  芯片的速度比SRAM  芯片的快
IV.DRAM   芯片工作时需要刷新，SRAM  芯片工作时不需要刷新
通常情况下，错误的是()。B
I    和 Ⅱ         B.Ⅱ    和 Ⅲ        C.Ⅲ  和IV          D.I  和IV
10.The following are descriptions about DRAM and SRAM memory :
...
The incorrect statements are ().
I &Ⅱ         B.Ⅱ&Ⅲ        C.Ⅲ&IV          D.I &IV
某一DRAM 芯片，采用地址复用技术，容量为1024×8位，该芯片的地址引脚和数据引
脚总数至少是()。B
A.18               B.13                 C.8                  D.17
11.A DRAM uses address multiplexing technology(地址复用技术） and has a capacity of 1024×8 bits. The minimum total number of its address pins and data pins is ().
A.18               B.13                 C.8                  D.17

【2012统考真题】下列关于闪存的叙述中，错误的是()。
A.  信息可读可写，并且读、写速度一样快
B. 存储元件由MOS 管组成，是一种半导体存储器
C. 掉电后信息不丢失，是一种非易失性存储器
D.  采用随机访问方式，可替代计算机外部存储器
12. Which of the following statements about Flash memory is WRONG?
A. Information can be read and written, and the read and write speeds are the same.
B. Its storage elements consist of MOS transistors, making it a semiconductor memory.
C. Information is not lost after power failure, making it a non-volatile memory.
D. It uses random access and can replace the computer's external memory.

 4个16K×8位的存储芯片，可设计为()容量的存储器。A
A.32K×16         位           B.16K×16          位      
  C.32K×8          位         D.8K×16          位
13.Four 16K×8 bit memory chips can be designed into a memory with a capacity of ().
A.32K×16      B.16K×16         C.32K×8          D.8K×16   

【2021统考真题】某计算机的存储器总线中有24位地址线和32位数据线，按字编址， 字长为32位。若000000H～3F FFFFH为RAM区，则需要512K×8位的RAM芯片数 为 (C ) 。
 A.8                 B.   16             C.   32                  D.64
14.A computer has 24-bit address bus and 32-bit data bus. It is word-addressable with a word length of 32 bits. If the address range 000000H～3FFFFFH is the RAM area, then the number of required 512K×8 bit RAM chips is ().
A.8        B.   16              C.   32             D.64

大题：
suppose the control signals from CPU are M/IO ,/RD,/WR, Using 16K×8 bits SRAM chips and 2-4 or 3-8 decoder to design a 64K×8bits memory module.
1. Draw a diagram of the connections for the CPU and memory chips, show all the required input and output signals.
2. Give the addressing range of each memory chip. 
...
D. The aim of formatting disk is to add additional information to mark tracks and sectors 
which is available to user 
2.On a movable head system, the time it takes for the beginning of the sector to rotate under the head is known as           .
A.  rotational delay    B. seek time     C.  transfer time   D. access time 
3. The time for the head of hard disk move to the correct track is ____________.
A. rotational delay    B. seek time    C.  transfer time   D.  access time 
4. In RAID, parity means that more disk’s capacity is used_____.
A. to enlarge capacity of available data in disk system  
B. to enable the recovery of data lost due to a disk failure 
C. to find and correct the read/write errors            
D. to improve I/O request rate of system
5. In RAID, REDUNDANT means that more disks are used for _____________.
...
6. RAID level(s)_________make(s) use of an independent access technique for the purpose of high I/O request rate.
A.  2   B. 3    C.  4    D. all 
7.RAID levels 2 and 3 make use of a _______technique for the purpose of high data transfer rate.
A. parallel access    B. random access   C. direct access   D.  independent access
8. Assume a 10-drive RAID configuration. In RAID1 level, storage density is _______
A. 90%   B.  80%     C. 60%   D. 50% 
9. Compared with RAID4, the advantage of RAID5 is _______
...
10. When a processor accesses disk, it must know three parameters: _____________.
A. side, head and track        B. cylinder and head, sector    
C. cylinder, sector and track    D. side, sector and platter
11. The method for accessing a unit of data on a disk is_______
A.Random Access  B. Read-Only   C. Sequential Access  D. Direct Access 
磁盘属于()类型的存储器。
...
2. Using stack to save PC and remainder, the reason is ____________________ .
...
3. In DMA, the DMA module takes over the operations of data transferring from CPU, it means _________________________
...
4. Transfer data with I/O modules, 3 types of techniques can be used. Which one is not belong them?
A. Interrupt-driven I/O    B. programmed I/O    C. direct I/O access   D. DMA
5. To input a word from keyboard, Which I/O technique should be taken?___________.
A. Interrupt-driven I/O    B. programmed I/O    C. direct I/O access   D. DMA
6. To output a data block of some sectors to hard disk, Which I/O technique should be taken? ___________
A. Interrupt-driven I/O    B. programmed I/O    C. direct I/O access   D. DMA
7. Comparing with interrupt-driven I/O, DMA further raises the usage rate of CPU operations, because __________
...
D. both a and b
8. Transfer data with I/O modules, 3 types of techniques can be used. Which isn’t necessary for CPU to intervene the dada transfer? 
A. Interrupt-driven I/O    B. programmed I/O    C. direct I/O access   D. DMA
9.Comparing with programmed I/O, interrupt-driven I/O further raises the usage rate of CPU operations, because __________
A it isn’t necessary for CPU to save & restore scene    
B it isn’t necessary for CPU to intervene the dada transfer
C it isn’t necessary for CPU to read & check status repeatedly    
D both a and b
10.In DMA, when does DMA module issue interrupt request signal to CPU?
