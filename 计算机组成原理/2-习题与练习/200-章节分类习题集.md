...
1. **Two's Complement Arithmetic / 补码运算**
    ...
    **中文：** 假设两个整数 x = -68 和 y = -80 用补码形式表示（包括 1 位符号位）。x 和 y 存储在寄存器 A 和 B 中。还有两个寄存器 C 和 D。寄存器 A、B、C、D 都是 8 位寄存器。请回答以下问题（最终答案用十六进制表示）：
    ...
    > 
    > **1) 寄存器 A 和 B 的内容：**
    > - x = -68 的补码：68 = 01000100B，-68 = 10111100B = BC H
    > - y = -80 的补码：80 = 01010000B，-80 = 10110000B = B0 H
    > - 寄存器 A = BC H，寄存器 B = B0 H
    > 
    > **2) x + y 的结果：**
    > - BC H + B0 H = 16C H，溢出舍去最高位，结果 = 6C H
    > - 寄存器 C = 6C H，OF = 1（发生溢出），SF = 0（符号位为0）
    > 
    > **3) x - y 的结果：**
    > - x - y = -68 - (-80) = 12
    > - 12 = 00001100B = 0C H
    > - 寄存器 D = 0C H，OF = 0（无溢出），SF = 0（符号位为0）
...
2. **C Program Execution in 8-bit Computer / 8位计算机中C程序执行**
    ...
    **中文：** 假设以下 C 程序在 8 位字长的计算机上执行：
    ...
    > **答案 (Answer):**
    > 
    > **1) 寄存器 R1、R5 和 R6 的内容：**
    > - R1 (x) = 134 = 86H
    > - R5 (z1) = x - y = 134 - 246 = -112（无符号）= 256 - 112 = 144 = 90H
    > - R6 (z2) = x + y = 134 + 246 = 380 = 256 + 124 = 124 = 7CH
    > 
    > **2) 变量 m 和 k1 的值：**
    > - m = x = 134（作为有符号数，86H = 134）
    > - n = y = 246 = F6H（作为有符号数，F6H = -10）
    > - k1 = m - n = 134 - (-10) = 144（有符号溢出，实际值为-112）
    > 
    > **3) 能否使用同一个加法器：**
    > - 可以。因为有符号数和无符号数的加减运算在硬件上使用相同的加法器电路，区别在于对结果的解释和标志位的设置。
    > 
    > **4) 溢出判断和溢出语句：**
    > - 判断方法：当两个同号数相加结果与操作数符号不同，或两个异号数相减结果与被减数符号不同时，发生溢出。
    > - 溢出语句：k2 = m + n（134 + (-10) = 124，无溢出），k1 = m - n（134 - (-10) = 144，发生溢出）
...
1. **ADD AX, X;**
   - **English:** The contents of AC adds the contents of location X, result is stored to AC.
   - **中文：** AC的内容加上存储单元X的内容，结果存入AC。

   > **答案 (Answer):**
   > 
   > **微操作序列 (Micro-operations):**
   > - T1: PC → MAR, (PC) + 1 → PC（取指）
   > - T2: M(MAR) → IR（指令进入IR）
   > - T3: X → MAR（地址部分进入MAR）
   > - T4: M(MAR) → MBR（操作数进入MBR）
   > - T5: (AC) + (MBR) → AC（执行加法）
   > 
   > **控制信号 (Control Signals):**
   > - T1: PCout, MARin, PCinc
   > - T2: Mread, IRin
   > - T3: IRout, MARin
   > - T4: Mread, MBRin
   > - T5: ACout, MBRout, ALUadd, ACin

2. **MOV AX, [X];**
   - **English:** Operand pointed by the content of location X is moved to AX, that means ((X))→AX. [ ] means indirect addressing.
   - **中文：** 将存储单元X的内容所指向的操作数移入AX，即((X))→AX。[ ]表示间接寻址。
   ...
   > - T2: M(MAR) → IR（指令进入IR）
   > - T3: X → MAR（地址X进入MAR）
   > - T4: M(MAR) → MBR（X处的内容进入MBR）
   > - T5: (MBR) → MAR（间接地址进入MAR）
   > - T6: M(MAR) → MBR（操作数进入MBR）
   > - T7: (MBR) → AX（操作数进入AX）
   > 
   > **控制信号 (Control Signals):**
   > - T1: PCout, MARin, PCinc
...
3. **ADD AX, [BX];**
   - **English:** Operand pointed by the content of Register BX is added to AX, that means (AX)+((BX))→AX. [ ] means register indirect addressing.
   - **中文：** 将寄存器BX的内容所指向的操作数加到AX，即(AX)+((BX))→AX。[ ]表示寄存器间接寻址。
