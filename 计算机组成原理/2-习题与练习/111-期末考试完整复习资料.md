...
> MESI 协议是一种基于**写失效 (Write-Invalidate)** 的缓存一致性协议。当一个核心修改数据时，它会通知其他核心将其副本标记为“无效 (Invalid)”，而不是更新它们。
>
...
> 典型的复杂指令周期包括：**取指 (Fetch)** -> **间址 (Indirect)** -> **执行 (Execute)**。这构成了 3 个主要周期。（注：有些教材可能加上中断周期算4个，但针对“执行过程”通常指前三个）。
>
> [14-指令执行周期.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/14-指令执行周期.md)
</details>
...
> 超标量架构的核心在于拥有多条并行的指令流水线，能够在一个时钟周期内发射并执行多条指令（Instruction Level Parallelism）。
>
> [16-流水线技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/16-流水线技术.md)
</details>
...
> RISC (精简指令集) 的特征是：指令简单、寻址方式少、格式统一，但它通常拥有**大量 (Large number)** 的通用寄存器，以减少对内存的访问。因此它“拒绝”少量寄存器的设计。
>
> [12-RISC与CISC.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/12-RISC与CISC.md)
</details>
...
> Arithmetic Logic Unit，顾名思义，负责算术运算和逻辑运算。
>
> [06-二进制运算.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/06-二进制运算.md)
</details>
...
> 操作数可以存储在寄存器、内存（包括 Cache 和虚拟内存）中。**移位器 (Shifter)** 是 ALU 的一部分，用于执行操作，而不是存储操作数的地方。
>
> [09-指令系统基础.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/09-指令系统基础.md)
</details>
...
> **条件分支 (Conditional Branch)** 会导致控制冒险 (Control Hazard)。如果预测失败，流水线需要被清空（Flush），造成严重的性能损失（Pipeline Stall）。
>
> [16-流水线技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/16-流水线技术.md)
</details>
...
> 直接寻址：指令中的地址字段直接给出操作数的有效地址 (EA = A)。
>
...
> 内存地址通常被视为**无符号整数 (Unsigned Integer)**，从 0 开始递增。
>
> [02-数据组织与表示.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/02-数据组织与表示.md)
</details>
...
> 溢出检测规则：正+正=负，或 负+负=正，即结果符号与操作数符号相反。
>
> [04-定点数表示.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/04-定点数表示.md)
</details>
...
> 间接周期用于读取内存中存放的**有效地址 (Effective Address)**，即操作数的真实地址。
>
> [14-指令执行周期.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/14-指令执行周期.md)
</details>
...
> RAID 的校验位（Parity）主要用于**容错 (Fault Tolerance)**，当一块盘坏掉时，利用校验信息重建数据。
>
> [21-辅助存储.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/21-辅助存储.md)
</details>
...
> RAID 2 和 RAID 3 使用同步旋转和条带化，属于并行访问。RAID 4, 5, 6 侧重于独立访问。
>
> [21-辅助存储.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/21-辅助存储.md)
</details>
...
> **PC (Program Counter)** 始终指向下一条要取出的指令。
>
> [13-CPU结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/13-CPU结构.md)
</details>
...
> Direct Memory Access，I/O 设备直接与内存交换数据，不经过 CPU。
>
> [24-DMA技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/24-DMA技术.md)
</details>
...
> 从体系结构角度，寄存器通常分为：**用户可见寄存器 (User-visible)**（如通用寄存器、数据/地址寄存器，程序员可用）和 **控制/状态寄存器 (Control/Status)**（如 PC, IR, MAR, MBR, PSW，由 CPU 内部控制使用）。
>
> [13-CPU结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/13-CPU结构.md)
</details>
...
> **存储程序 (Stored Program)** 概念：指令和数据一样存储在内存中，并且可以被读写和处理。
>
> [01-计算机体系结构概述.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/01-计算机体系结构概述.md)
</details>
...
> PC 存放的是内存地址。因此 PC 的位数决定了 CPU 能寻址的最大内存空间（Address Space）。例如 32 位 PC 可以寻址 4GB 内存。
>
> [13-CPU结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/13-CPU结构.md)
</details>
...
> *   所以需要 15 条地址线。
>
> [18-存储层次结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/18-存储层次结构.md)
</details>
...
> 虚拟内存的主要目的是给程序员提供一个比实际物理内存大得多的**逻辑地址空间**（Enlarge logical capacity），让程序以为自己拥有很大的内存。
>
> [18-存储层次结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/18-存储层次结构.md)
</details>
...
> Cache 的存在就是为了缓解 CPU 和主存之间的速度差异（Memory Wall）。如果速度匹配，Cache 就失去了存在的意义。
>
> [18-存储层次结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/18-存储层次结构.md)
</details>
...
> 基础总线类型是：数据、地址、控制。系统总线是它们的集合统称。
>
> [22-总线系统.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/22-总线系统.md)
</details>
...
> 指令译码器 (Instruction Decoder) 的作用是将指令寄存器 (IR) 中的操作码 (Opcode) 翻译成控制信号，传送给控制单元 (Control Unit) 执行。因此它位于 IR 和 CU 之间。
>
> [13-CPU结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/13-CPU结构.md)
</details>
...
> 完整的指令周期通常包含 4 个子周期：**取指 (Fetch)**、**间址 (Indirect)**、**执行 (Execute)**、**中断 (Interrupt)**。
>
> [14-指令执行周期.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/14-指令执行周期.md)
</details>
...
> 冯·诺伊曼 (John von Neumann) 提出了存储程序概念。
>
> [01-计算机体系结构概述.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/01-计算机体系结构概述.md)
</details>
...
> 理想情况下，流水线的加速比与**级数 (Number of stages)** 成正比。级数越多（深度越深），潜在的主频越高，吞吐率越高（但也受限于冒险和开销）。
>
> [16-流水线技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/16-流水线技术.md)
</details>
...
> ALU (Arithmetic Logic Unit) 负责算术运算 (Arithmetic) 和逻辑运算 (Logic)。
>
> [13-CPU结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/13-CPU结构.md)
</details>
...
> 虽然 I/O 端口可以被寻址，但在通用指令集中，操作数通常直接来自**寄存器**或**内存**。I/O 访问通常需要专门的指令 (IN/OUT) 或通过内存映射 I/O 视为内存地址，而不是作为普通算术指令的直接操作数位置（相比之下，Register 和 Memory 是最标准的）。*注：此题有一定歧义，但在常见选项中，ALU 不能直接对 I/O 设备进行算术运算，必须先读入寄存器。*
>
> [09-指令系统基础.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/09-指令系统基础.md)
</details>
...
> **条件分支 (Conditional Branch)** 引起的控制冒险 (Control Hazard) 会导致流水线断流 (Flush)，造成严重的性能损失。
>
> [16-流水线技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/16-流水线技术.md)
</details>
...
> RAID (Redundant Array of Independent Disks) 的核心优势之一是**冗余 (Redundancy)**，用于数据恢复。
>
> [21-辅助存储.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/21-辅助存储.md)
</details>
...
> RAID 5 使用**分布式奇偶校验 (Distributed Parity)**，将校验块分散存储在所有磁盘上，避免了 RAID 4 的校验盘瓶颈。
>
> [21-辅助存储.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/21-辅助存储.md)
</details>
...
> 直接寻址的地址字段位数固定，限制了可寻址的内存空间大小（例如 8 位地址只能寻址 256 字节）。
>
> [10-寻址方式.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/10-寻址方式.md)
</details>
...
> 主存（RAM）是**随机存取存储器 (Random Access Memory)**，意味着访问任意地址的时间是相同的。
>
> [18-存储层次结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/18-存储层次结构.md)
</details>
...
> DRAM (Dynamic RAM) 使用电容存储电荷来表示数据，结构简单（1T1C），密度高，但需要刷新。SRAM 使用触发器（6T）。
>
> [18-存储层次结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/18-存储层次结构.md)
</details>
...
> CPU 在指令执行完毕后，进入**中断周期 (Interrupt Cycle)** 检查中断请求并发送确认信号（Interrupt Acknowledge）。
>
> [25-中断系统.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/25-中断系统.md)
</details>
...
> **写通 (Write-Through)**：每次写 Cache 时，同时也写主存，保持两者一致。
>
> [19-Cache技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/19-Cache技术.md)
</details>
...
> 数据移动（Data Movement / Transfer）指令负责在 CPU（寄存器）和内存之间传送数据（如 LOAD, STORE）。
>
> [11-指令分类.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/11-指令分类.md)
</details>
...
> **L1 Cache** 通常采用**哈佛架构 (Harvard Architecture)**，即分为独立的 I-Cache 和 D-Cache，以支持指令流水线并行读取指令和数据。L2/L3 通常是统一的 (Unified)。
>
> [19-Cache技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/19-Cache技术.md)
</details>
...
> DMA 控制器发送 **Bus Request (总线请求)** 或 **Hold Request**，通常也称为 DMA 请求，以获得总线控制权。
>
> [24-DMA技术.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/24-DMA技术.md)
</details>
...
> SRAM 速度快但昂贵，主要用于 Cache（无论是集成在 CPU 内部的 L1/L2 还是外部的 L3）。
>
> [18-存储层次结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/18-存储层次结构.md)
</details>
...
> 取指周期结束后，MBR 中存放的是刚刚取出的**指令**。虽然对于立即寻址，指令中包含了操作数，但在取指阶段，CPU 只是把它当作指令取进来。
>
> [14-指令执行周期.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/14-指令执行周期.md)
</details>
...
> 海明码 (Hamming Code) 是纠错码 (Error Correcting Code)，可以检测并纠正单比特错误 (SEC)。
>
> [08-舍入与误差.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/08-舍入与误差.md)
</details>
...
> **MAR (Memory Address Register)** 用于存放即将访问的内存单元地址。
>
> [13-CPU结构.md](file:///d:/我的笔记/计算机组成原理/1-核心笔记/精简版/13-CPU结构.md)
</details>
