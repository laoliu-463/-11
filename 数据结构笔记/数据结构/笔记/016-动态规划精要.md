...
## 📚 相关：[时间复杂度精要](./009-时间复杂度精要.md) | [图精要](./011-图精要.md) | [代码练习](../代码练习/06-动态规划/) | [思维导图](./数据结构思维导图.md)
...
int Knapsack(int weights[], int values[], int n, int capacity) {
    ...
    // 初始化：dp[0][j] = 0, dp[i][0] = 0（calloc已初始化为0）
...
// 空间优化：一维数组
int KnapsackOptimized(int weights[], int values[], int n, int capacity) {
...
// 回溯找具体方案
// 返回选择的物品索引数组，count为返回的物品数量
int* KnapsackWithPath(int weights[], int values[], int n, int capacity, int *count) {
    // 动态分配二维数组
    int **dp = (int **)malloc((n + 1) * sizeof(int *));
    for (int i = 0; i <= n; i++) {
        dp[i] = (int *)calloc(capacity + 1, sizeof(int));
    ...
    *count = 0;
    ...
    while (i > 0 && j > 0) {
        ...
        if (dp[i][j] == dp[i - 1][j]) {
            ...
            // 选了第i个物品
            selected[(*count)++] = i - 1;  // 物品编号（0-indexed）
            j -= weights[i - 1];
            i--;
        }
    }
...
int main() {
    ...
    // 找具体方案
    int count;
    int *selected = KnapsackWithPath(weights, values, n, capacity, &count);
...
// 优化：二分查找 O(n log n)
int LengthOfLISOptimized(int nums[], int n) {
    if (n == 0) return 0;
