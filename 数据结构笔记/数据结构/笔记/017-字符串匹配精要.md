...
## 📚 相关：[查找精要](./013-查找精要.md) | [时间复杂度精要](./009-时间复杂度精要.md) | [代码练习](../代码练习/07-字符串匹配/) | [思维导图](./数据结构思维导图.md)
 ...
 * @return 所有匹配位置的数组
 */
int* FindAll(char *text, char *pattern, int *count) {
    int n = strlen(text);
    int m = strlen(pattern);
    *count = 0;
...
int main() {
    ...
    int count;
    int *allPos = FindAll(text, "aba", &count);
    printf("所有匹配位置: [");
...
int* BuildNext(char *pattern) {
    ...
    while (i < m) {
        if (j == -1 || pattern[i] == pattern[j]) {
            ...
            i++;
            j++;
            if (i < m) next[i] = j; // 注意边界检查，虽然原逻辑隐含
        } else {
            // 不匹配，回溯到前一个可能的位置
            j = next[j];
        }
    }
    // 修正：上面的循环逻辑有点小问题，标准KMP构建如下：
    // 重置
    next[0] = -1;
    i = 0; j = -1;
    while (i < m - 1) {
 ...
 * @return 所有匹配位置的数组
 */
int* FindAllKMP(char *text, char *pattern, int *count) {
    int n = strlen(text);
    int m = strlen(pattern);
    *count = 0;
    ...
    while (i < n) {
        ...
        if (j == m) {
            ...
            // 注意：这里通常回退到 next[m] 如果 next 数组长度为 m+1
            // 或者根据具体实现调整。简单起见，这里假设 next 数组足够大或逻辑调整
            // 实际上标准KMP找到匹配后，j应该置为 next[j] (即next[m])
            // 但我们的next数组只到m-1。
            // 重新审视BuildNext，它生成长度为m的数组。
            // 实际上我们需要next[m]来处理这种情况，或者手动处理。
            // 这里简单处理：
            j = next[m - 1]; 
            // 修正：如果完全匹配，j=m，next数组访问越界。
            // 实际上应该在BuildNext时多分配一个空间或者特殊处理。
            // 为了简化，这里我们让 i 回退一步，j 重置（虽然效率低点但正确）
            // 或者更标准的做法是 BuildNext 生成 m+1 长度。
            // 让我们修改 BuildNext 生成 m+1 长度。
        }
    }
...
// 重新定义 BuildNext 以支持 FindAllKMP 的需求（生成 m+1 长度）
int* BuildNextStandard(char *pattern) {
    int m = strlen(pattern);
 ...
 * 打印next数组（用于调试）
 */
void PrintNext(char *pattern) {
    int *next = BuildNextStandard(pattern);
    int m = strlen(pattern);
...
int main() {
    ...
    int count;
    // 注意：FindAllKMP 需要使用 BuildNextStandard
    // 这里为了演示简单，我们假设 FindAllKMP 内部使用了正确的 BuildNext
    // 实际代码中应该统一。
...
```c
int KMPOptimized(char *text, char *pattern) {
    int n = strlen(text);
    int m = strlen(pattern);
