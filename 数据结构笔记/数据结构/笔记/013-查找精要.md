...
public class SequentialSearch {
    ...
    // 顺序查找（带详细注释）
    public static int sequentialSearchDetailed(int[] arr, int target) {
        int n = arr.length;
        ...
        for (int i = 0; i < n; i++) {
            System.out.printf("比较 arr[%d]=%d 和 target=%d", i, arr[i], target);
    ...
    // 顺序查找（在有序表中，可以提前终止）
    public static int sequentialSearchOrdered(int[] arr, int target) {
        int n = arr.length;
...
public static int sequentialSearchWithSentinel(int[] arr, int target) {
    ...
    extendedArr[n] = target;  // 哨兵
    
    int i = 0;
    while (extendedArr[i] != target) {
        i++;
    }
...
```java
// 顺序查找（有序表，提前终止）
public static int sequentialSearchOrderedOptimized(int[] arr, int target) {
    int n = arr.length;
