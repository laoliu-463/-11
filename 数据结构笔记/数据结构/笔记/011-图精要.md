...
public class CriticalPath {
    ...
    public CriticalPathResult findCriticalPath() {
        ...
        int source = topoOrder.get(0);  // 源点（拓扑序列第一个）
        int sink = topoOrder.get(topoOrder.size() - 1);  // 汇点（拓扑序列最后一个）
        ...
        Arrays.fill(vl, ve[sink]);  // 初始化为汇点的最早发生时间
        ...
        Collections.reverse(topoOrder);  // 恢复拓扑顺序
        ...
        for (int u = 0; u < vertexCount; u++) {
            ...
            for (int v : adjacent) {
                ...
                if (e == l) {
                    criticalActivities.add(activity);  // 关键活动
                }
            }
        }
...
class Activity {
    ...
    public String toString() {
        return String.format("活动<%d,%d>: 持续时间=%d, e=%d, l=%d, %s",
            from, to, weight, e, l, isCritical() ? "关键活动" : "非关键活动");
    }
}
...
class CriticalPathResult {
    int[] ve;                      // 事件最早发生时间
    int[] vl;                      // 事件最迟发生时间
    List<Activity> activities;     // 所有活动
    List<Activity> criticalActivities;  // 关键活动
    int totalTime;                 // 总工期
    ...
    void print() {
        ...
        System.out.println("总工期: " + totalTime);
        System.out.println("\n事件最早发生时间（ve）:");
