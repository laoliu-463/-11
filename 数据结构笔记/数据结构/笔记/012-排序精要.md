...
public class BinaryInsertionSort {
    ...
    // 更清晰的实现
    public static void binaryInsertionSortV2(int[] arr) {
        int n = arr.length;
...
public class ShellSort {
    ...
    // 带详细注释的版本
    public static void shellSortDetailed(int[] arr) {
        int n = arr.length;
        ...
        for (int gap = n / 2; gap > 0; gap /= 2) {
            ...
            for (int i = gap; i < n; i++) {
                ...
                System.out.printf("  处理元素 arr[%d] = %d\n", i, key);
                
                // 在组内进行插入排序
                while (j >= gap && arr[j - gap] > key) {
                    arr[j] = arr[j - gap];
                    j -= gap;
                    System.out.printf("    移动: arr[%d] = arr[%d]\n", j + gap, j);
                }
                
                arr[j] = key;
                System.out.printf("    插入: arr[%d] = %d\n", j, key);
    ...
    public static void shellSortSedgewick(int[] arr) {
        ...
        // 找到合适的起始增量
        int gapIndex = 0;
        while (gaps[gapIndex] < n / 3 && gapIndex < gaps.length - 1) {
            gapIndex++;
        }
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int j = 0; j < n - 1 - i; j++) {
                ...
                if (arr[j] > arr[j + 1]) {
                    swap(arr, j, j + 1);
                    swapped = true;
                    System.out.printf(" → 交换\n");
                } else {
                    System.out.printf(" → 不交换\n");
                }
            }
            
            System.out.println("  本轮结果: " + Arrays.toString(arr));
...
public class QuickSort {
    ...
    private static void quickSortHoare(int[] arr, int left, int right) {
        if (left < right) {
...
public class SelectionSort {
    ...
    public static void selectionSortDetailed(int[] arr) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            System.out.printf("  找到最小值 arr[%d] = %d\n", minIndex, arr[minIndex]);
            
            // 交换
            if (minIndex != i) {
                swap(arr, i, minIndex);
                System.out.printf("  交换 arr[%d] 和 arr[%d]\n", i, minIndex);
            } else {
                System.out.println("  已在正确位置，无需交换");
            }
...
public class HeapSort {
    ...
    // 带详细注释的版本
    public static void heapSortDetailed(int[] arr) {
        int n = arr.length;
        ...
        System.out.println("\n=== 构建大顶堆 ===");
        for (int i = n / 2 - 1; i >= 0; i--) {
            System.out.printf("调整节点 arr[%d] = %d\n", i, arr[i]);
            heapify(arr, n, i);
            System.out.println("当前状态: " + Arrays.toString(arr));
        }
        System.out.println("堆构建完成: " + Arrays.toString(arr));
...
public class MergeSort {
    ...
    // 带详细注释的版本
    public static void mergeSortDetailed(int[] arr) {
        if (arr == null || arr.length <= 1) {
            return;
        }
        mergeSortDetailed(arr, 0, arr.length - 1, 0);
    }
    
    private static void mergeSortDetailed(int[] arr, int left, int right, int depth) {
        String indent = "  ".repeat(depth);
        System.out.printf("%s归并排序 [%d...%d]\n", indent, left, right);
        ...
        if (left < right) {
            ...
            System.out.printf("%s合并 [%d...%d] 和 [%d...%d]\n", 
                indent, left, mid, mid + 1, right);
            merge(arr, left, mid, right);
            System.out.printf("%s结果: %s\n", indent, 
...
public class CountingSort {
    ...
    // 带详细注释的版本
    public static void countingSortDetailed(int[] arr, int maxValue) {
        int n = arr.length;
        
        System.out.println("初始数组: " + Arrays.toString(arr));
        System.out.println("数据范围: [0, " + maxValue + "]");
        ...
        System.out.println("\n步骤2：计算前缀和");
        System.out.println("前缀和数组: " + Arrays.toString(count));
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            output[pos] = num;
            count[num]--;
            System.out.printf("  处理 arr[%d]=%d，放入位置%d，结果: %s\n",
                i, num, pos, Arrays.toString(output));
        }
